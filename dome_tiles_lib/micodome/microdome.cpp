...
MicroDome::MicroDome(uint16_t index, int16_t Pin, ledDirType t)
...
/*!
  ...
  @note    This function is deprecated, here only for old projects that
           ...
           type).
*/
void MicroDome::updateLength(uint16_t index) {
  free(pixels); // Free existing data (if any)

  // Allocate new data -- note: ALL PIXELS ARE CLEARED
  numBytes = index * ((wOffset == rOffset) ? 3 : 4);
...
void MicroDome::updateType(ledDirType t) {
...
*/
void MicroDome::setPin(int16_t Pin) {
  if (begun && (pin >= 0))
...
void MicroDome::setTile(uint16_t index, uint32_t levels) {
  if (index < numLEDs) {
    uint8_t *p, r = (uint8_t)(levels >> 16), g = (uint8_t)(levels >> 8), b = (uint8_t)levels;
...
uint32_t MicroDome::getLedLevels(uint16_t index) const {
  if (index >= numLEDs)
    return 0; // Out of bounds, return no level.
...
void MicroDome::setBrightness(uint8_t b) {
...
*/
uint8_t MicroDome::getBrightness(void) const { return brightness - 1; }

/*!
  @brief   Fill the whole NeoPixel strip with 0 / black / off.
*/
void MicroDome::clear(void) { memset(pixels, 0, numBytes); }
